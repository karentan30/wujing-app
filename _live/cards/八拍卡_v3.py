...
def main():
    ...
    PHRASE_H = TOP_H + FR_H + 10  # 信息条 + 帧图 + 顶部内边距
    ...
    d.text((PAD, 8), f"《{title}》·  八拍卡", font=F(28, bold=True), fill=(255, 255, 255))
    d.text((PAD, 42), f"口诀 · 动作  ——  {len(ph)} 句完整拆解",
    ...
    for idx, (p, frames) in enumerate(strips):
        ...
        d.rectangle([0, y, 6, y + TOP_H], fill=col)  # 左色条

        # 核心大字（font 68，不与文字列竞争）
